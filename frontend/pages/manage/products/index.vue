...
const deleteProduct = async (productId: number) => {
  if (confirm('Bạn có chắc chắn muốn xóa sản phẩm này không?')) {
    const { error } = await useClientFetch(`shop/products/${productId}`, {
      method: 'DELETE'
    })
...
<template>
  ...
  <div class="bg-white rounded-xl p-4 mt-5 min-h-[calc(100vh-9.5rem)]">
    <div class="flex flex-wrap items-center gap-4">
      <NuxtLink :to="`/manage/products/${null}`" class="px-4 py-2 bg-blue-500 text-white rounded-lg"
    ...
    <div class="w-full overflow-x-auto">
      <div class="min-w-[800px]">
        ...
        <div v-if="data?.data?.length > 0" class="mb-5">
          ...
          >
            <div class="flex items-center">
              <NuxtLink :to="`/manage/products/${product.id}`" class="ml-2 text-blue-500 hover:underline">{{
                product.id
              }}</NuxtLink>
            </div>
            <div class="py-[5px]">
            ...
            <div>
              <button @click="deleteProduct(product.id)" class="text-red-500 p-3">
