...
<template>
  ...
  <div class="bg-white rounded-xl p-4 mt-5 min-h-[calc(100vh-9.5rem)] space-y-5">
    ...
    <div class="w-full overflow-x-auto">
      <div class="min-w-[900px]">
        ...
        <div v-if="data?.orders?.data?.length">
          <div
            v-for="ord_detail in data?.orders?.data"
            class="p-4 grid grid-cols-7 gap-4 items-center text-sm text-gray-700 border-b border-gray-200"
          >
            <div class="flex items-center">
              <NuxtLink :to="`/manage/orders/${ord_detail.id}`" class="ml-2 text-blue-500 hover:underline">{{
                ord_detail.id
              }}</NuxtLink>
            </div>
            <div>
              <span
                :class="statuses.find((status) => status.id === ord_detail.status).color"
    ...
    <AdminPagination v-if="data?.orders" :pagination_data="data?.orders" @page-change="goToPage" />
  </div>
</template>
